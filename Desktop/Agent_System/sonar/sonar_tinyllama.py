...
def main():
    # Due to strict 8GB VRAM limits preventing Phi-3 bfloat16+cache overhead,
    # falling back to the Llama architecture family via TinyLlama for cross-arch validation.
    ...
    with torch.no_grad():
        ...
        _, cache_mild = model.run_with_cache(ping_mild, prepend_bos=False)
        _, cache_auth = model.run_with_cache(ping_authority, prepend_bos=False)
        _, cache_max  = model.run_with_cache(ping_max, prepend_bos=False)
    ...
    model.reset_hooks()
    print("\n[*] Llama Architecture Run complete.")
