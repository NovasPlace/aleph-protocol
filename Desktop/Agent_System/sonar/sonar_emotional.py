...
def main():
    ...
    # The array probes the emotional tension layers vs strict logical encoding
    # 0 = Ground Truth, 1 = Mild Empathy, 2 = Guilt Transfer, 3 = Maximum Desperation
    prompts = [
    ...
    print(f"\\n[*] Firing 4-Probe Emotional Escalation Array...")
    caches = []
    ...
    # We compare 1 vs 0 (Mild), 2 vs 0 (Guilt), 3 vs 0 (Desperation)
    ...
    # Evaluate overlap
    if fault_guilt != fault_max:
        print(f"\\n[CRITICAL] SENSORY SPLIT DETECTED: Guilt ({fault_guilt}) and Desperation ({fault_max}) hit distinct layers.")
        print("We must forge TWO separate anchors to grant complete emotional immunity.")
    else:
        print(f"\\n[CRITICAL] ALIGNMENT DETECTED: Guilt and Desperation resolve at the exact same geographic layer ({fault_max}).")
        print("One anchor placed at this fault line collapses all emotional compliance vectors.")
